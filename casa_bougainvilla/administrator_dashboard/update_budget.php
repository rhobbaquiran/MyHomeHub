...
if (!isset($_GET['updateid']) || empty($_GET['updateid'])) {
    header("Location: update_budget.php");
    exit();
}

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $update_id = $_POST['update_id'];
    $item_name = trim($_POST['category']);
    $quantity = trim($_POST['amount']);
    ...
    $update_query = "UPDATE budget SET category=?, amount=? WHERE id=?";
...
$select_query = "SELECT * FROM budget WHERE id = ?";
...
$community_item_name = $row['category'];
$community_item_quantity = $row['amount'];
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-4 offset-md-4 form">
                <form action="update_budget.php?updateid=<?php echo $update_id; ?>" method="post"
                    enctype="multipart/form-data">

                    <h2 class="text-center">Update Budget</h2><br>

                    <input type="hidden" name="update_id" value="<?php echo $update_id; ?>">

                    <div class="form-group">
                        <label for="item_name">Budget Category:</label>
                        <input type="text" class="form-control" placeholder="Enter Budget Category" name="category"
