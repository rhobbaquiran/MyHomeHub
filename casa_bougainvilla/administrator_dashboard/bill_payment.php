...
if (isset($_POST['add_submit'])) // add_submit is the name of the button
{
    ...
    if (empty($errors)) {
        ...
        if ($insertPaymentResult) {
            ...
            $insertActivityQuery = "INSERT INTO activity_logs (user, timestamp, action, condominium_id) 
                                    VALUES ('$username', NOW(), '$action', '$condominium_id')";
            ...
            VALUES ('$username', NOW(), '$action', NULL)";
...
$sql = "SELECT bill_number, billing_period_start, billing_period_end, total_amount_due FROM admin_transactions WHERE account_number = '$account_number' AND status = 'Pending' AND is_deleted = 0";
