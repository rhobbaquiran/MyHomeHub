...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $update_query_admin = "UPDATE resident_transactions SET status = ? WHERE bill_number = ?";
    ...
    if ($new_status === 'Rejected' || $new_status === 'Verified')
    {
        if ($stmt_select_user_info->fetch()) 
        {
            ...
            $select_billing_info_query = "SELECT account_number, bill_number, billing_period_start, billing_period_end, due_date, total_amount_due FROM resident_transactions WHERE bill_number = $bill_number";
