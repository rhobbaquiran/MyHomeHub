...
if (isset($_POST['searchButton'])) {
    ...
    if (!empty($searchInput)) {
        ...
        $search_query = "SELECT u.*, c.name AS condominium_name 
                        ...
                        WHERE (u.account_number LIKE ? OR u.username LIKE ? OR u.email LIKE ? OR c.name LIKE ? OR u.role LIKE ?)
                        AND u.role = 'Front Desk' AND c.name = 'Casa Bougainvilla'
...
if (isset($_GET['redeploy_id'])) {
    ...
    if ($stmt->affected_rows > 0) {
        $_SESSION['success'] = 'Front Desk Reinstated Successfully.';
        // Log the activity
        logActivity($_SESSION['username'], "Reinstated Front Desk: $name. Reason: $reinstatement_reason");
        ...
        $_SESSION['error'] = 'Error Reinstated Front Desk: ' . $stmt->error;
    }
...
$sql = "SELECT u.*, c.name AS condominium_name 
        FROM users u
        LEFT JOIN condominiums c ON u.condominium_id = c.id
        WHERE u.role = 'Front Desk' AND c.name = 'Casa Bougainvilla'
        LIMIT $offset, $rowsPerPage";
...
<body>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="reportModalLabel">Front Desks Suspended Report</h5>
                            ...
                            <tbody id="suspendedAccountsTableBody">
                                ...
                                if ($resultUsers && $resultUsers->num_rows > 0) {
                                    ...
                                    while ($rowUser = $resultUsers->fetch_assoc()) {
                                        ...
                                        echo "<td style='white-space: nowrap; text-align: center;'>{$rowUser['suspend_timestamp']}</td>";
                                        echo "</tr>";
                                    ...
                                    echo "<div style='text-align: left; color: red;'><strong>Percentage of Suspended Front Desks: " . number_format($percentageSuspended, 2) . "%</strong></div>";
                                    echo "&nbsp;<div style='text-align: left; color: green;'><strong>Percentage of Non-Suspended Front Desks: " . number_format($percentageNonSuspended, 2) . "%</strong></div>";
                                    echo "</div>";
                                } else {
    ...
    <div class="container">
        <button class="btn btn-primary mx-5 my-5"><a href="add_employee.php" class="text-light">Add Front
                Desk</a></button>
        ...
        <div class="print-section">
            ...
            if (isset($search_result) && $search_result->num_rows > 0) {
                ...
                while ($row = $search_result->fetch_assoc()) {
                    ...
                    // Update button
                    echo '<button class="btn btn-primary"><a href="update_employee.php?updateid=' . $id . '" class="text-light">Update</a></button>';
                    ...
                    if ($suspended == 1) {
                        ...
                        echo '<button class="btn btn-danger"><a href="suspend_employee.php?suspendid=' . $id . '" class="text-light">Suspend</button></td></tr>';
                    }
            ...
            <h2 class="mt-4 mb-3" style="white-space: nowrap; text-align: center;">Front Desks List</h2>
    ...
    <script>
        ...
        document.querySelectorAll('.action-buttons .btn-danger').forEach(button => {
            button.addEventListener('click', function () {
                ...
                window.location = "suspend_employee.php?suspendid=" + id;
            });
        });
        ...
        function confirmRedeploy(id) {
            ...
            if (reinstateReason.trim() === "") {
                ...
                let confirmMessage = "Are you sure you want to Reinstate this Front Desk?\n\nReinstatement Reason: " + reinstateReason;
                if (confirm(confirmMessage)) {
                    window.location = "employees.php?redeploy_id=" + id + "&reinstatement_reason=" + encodeURIComponent(reinstateReason);
                }
            }
        }
