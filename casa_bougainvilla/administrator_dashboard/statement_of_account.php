...
if (isset($_POST['searchButton'])) {
    ...
    if (!empty($searchInput) && !empty($_SESSION['account_number'])) {
        ...
        $search_query = "SELECT * FROM admin_transactions WHERE (account_number LIKE ? OR condominium LIKE ? OR bill_number LIKE ? OR status LIKE ?  OR billing_period_start LIKE ? OR billing_period_end LIKE ? OR due_date LIKE ?) AND is_deleted = 0 AND account_number = ?";
