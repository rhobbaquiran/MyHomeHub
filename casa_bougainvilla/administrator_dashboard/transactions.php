...
if (isset($_POST['searchButton'])) {
    ...
    if (!empty($searchInput)) {
        ...
        $search_query = "SELECT * FROM resident_transactions WHERE (account_number LIKE ? OR username LIKE ? OR bill_number LIKE ? OR status LIKE ?  OR billing_period_start LIKE ? OR billing_period_end LIKE ? OR due_date LIKE ?) AND is_deleted = 0 ORDER BY FIELD(status, 'Pending') DESC, id DESC LIMIT $offset, $rowsPerPage";
...
if (isset($_GET['deleteid'])) {
    ...
    $select_query = "SELECT bill_number, username FROM resident_transactions WHERE id = ? AND is_deleted = 0";
    ...
    $update_query = "UPDATE resident_transactions SET is_deleted = 1 WHERE id = ? AND is_deleted = 0";
    ...
    if ($stmt->affected_rows > 0) {
        ...
        logActivity($_SESSION['username'], "Deleted transaction for resident: $username with Bill Number: $bill_number", $condominium_id);
    } else {
    ...
    <div class="container">
        ...
        <div class="print-section">
            ...
            <div class="list-of-sales second-table">
                <h2 class="mt-4 mb-3" style="white-space: nowrap; text-align: center;">Residents' Bills</h2>
                <table id="TableSorter2" class="table col-mx-5">
                    ...
                    <tbody>
                        ...
                        $sql = "SELECT id, account_number, username, bill_number, billing_period_start, billing_period_end, due_date, total_amount_due, status FROM resident_transactions WHERE is_deleted = 0 ORDER BY FIELD(status, 'Pending') DESC, id DESC LIMIT $offset, $rowsPerPage";
            ...
            <ul class="pagination justify-content-center">
                <?php
                $totalRows = $mysqli->query("SELECT COUNT(*) as count FROM resident_transactions WHERE is_deleted = 0")->fetch_assoc()['count'];
