...
if (isset($_POST['searchButton'])) {
    ...
    if (!empty($searchInput) && !empty($_SESSION['username'])) {
        ...
        $search_query = "SELECT * FROM payments WHERE (timestamp LIKE ? OR username LIKE ? OR bill_number LIKE ? OR screenshot LIKE ?  OR status LIKE ?) AND is_deleted = 0 AND username = ?";
        ...
        $stmt_search->bind_param("ssssss", $searchInput, $searchInput, $searchInput, $searchInput, $searchInput, $_SESSION['username']);
        $stmt_search->execute();
...
$sql = "SELECT id, timestamp, username, bill_number, screenshot, status 
        FROM payments 
        WHERE is_deleted = 0 AND username = ? 
        ORDER BY FIELD(status, 'Unverified') DESC, timestamp DESC 
        LIMIT ?, ?";
$stmt = $mysqli->prepare($sql);
$stmt->bind_param("sii", $_SESSION['username'], $start_from, $results_per_page);
$stmt->execute();
...
<body>
    ...
    <div class="container">
        ...
        <div class="container mt-3">
            <div class="row">
                <div class="col-md-6 offset-md-3">
                    <form method="post" action="account_payment_history.php">
        ...
        <div class="print-section">
            ...
            <th scope="row"><center>' . $timestamp . '</center></th>
            ...
            <td style="white-space: nowrap; text-align: center;">
                <center><a href="' . $screenshot . '" download="' . basename($screenshot) . '">' . basename($screenshot) . '</a></center>
            </td>
            ...
            <div class="list-of-sales second-table">
                <h2 class="mt-4 mb-3" style="white-space: nowrap; text-align: center;">Account Payment History</h2>
                <table id="TableSorter3" class="table col-mx-5">
                    ...
                    <tbody>
                        ...
                        $sql = "SELECT id, timestamp, username, bill_number, screenshot, status, rejection_reason 
                        FROM payments 
                        WHERE is_deleted = 0 AND username = ? 
                        ORDER BY FIELD(status, 'Unverified') DESC, timestamp DESC 
                        LIMIT ?, ?";
                        ...
                        if ($stmt) {
                            ...
                            // Bind parameters
                            $stmt->bind_param("sii", $username, $offset, $rowsPerPage);
        ...
        <div class="pagination justify-content-center mt-3">
            <ul class="pagination">
                ...
                for ($i = 1; $i <= $total_pages; $i++) {
                    echo '<li class="page-item ' . ($i == $current_page ? 'active' : '') . '"><a class="page-link" href="account_payment_history.php?page=' . $i . '">' . $i . '</a></li>';
                }
                ?>
            </ul>
        </div>
