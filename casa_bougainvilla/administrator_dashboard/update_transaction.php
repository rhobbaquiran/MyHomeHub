...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $selectedResident = trim($_POST['selected_resident']);
    ...
    $update_query = "UPDATE resident_transactions SET billing_period_start=?, billing_period_end=?, due_date=?, total_amount_due=?, status=?, account_number=?, username=?, bill_number=? WHERE id=?";
    ...
    {
        logActivity($_SESSION['username'], "Updated transaction for resident: $residentUsername with Bill Number: $billNumber", $residentUsername, $billNumber);
...
$select_query = "SELECT * FROM resident_transactions WHERE id = ?";
...
$residentQuery = "SELECT users.account_number, users.username FROM users WHERE users.role = 'Resident'";
            ...
            <div class="col-md-4 offset-md-4 form">
                <form action="update_transaction.php?updateid=<?php echo $updateId; ?>" method="post"
                    ...
                    <div class="form-group">
                        <label for="selected_resident">Select Resident:</label>
                        <select class="form-control" name="selected_resident" required>
                            <?php
