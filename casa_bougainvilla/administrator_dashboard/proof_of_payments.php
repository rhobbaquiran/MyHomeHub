...
function logActivity($user, $action, $condominium_id)
{
    ...
    $insert_query = "INSERT INTO activity_logs (timestamp, user, action, condominium_id,account_number) VALUES (CURRENT_TIMESTAMP, ?, ?, ?,$account_number)";
...
if (isset($_POST['searchButton'])) {
    ...
    if (!empty($searchInput) && !empty($_SESSION['username'])) {
        ...
        $search_query = "SELECT p.id, p.timestamp, p.username, p.bill_number, p.screenshot, p.status, p.rejection_reason
                 ...
                 AND p.is_deleted = 0 AND p.condominium_id = ? AND u.role = 'Resident'";
        ...
        $stmt_search->bind_param("sssssi", $searchInput, $searchInput, $searchInput, $searchInput, $searchInput, $condominium_id);
...
$sql = "SELECT p.id, p.timestamp, p.username, p.bill_number, p.screenshot, p.status, p.rejection_reason
        FROM payments p
        INNER JOIN users u ON p.username = u.username
        WHERE p.is_deleted = 0 AND p.condominium_id = ? AND u.role = 'Resident'
        ORDER BY FIELD(p.status, 'Unverified') DESC, p.timestamp DESC 
        LIMIT ?, ?";

$stmt = $mysqli->prepare($sql);
$stmt->bind_param("iii", $condominium_id, $start_from, $results_per_page);
$stmt->execute();
...
<body>
    ...
    <div class="container">
        ...
        LIMIT ?, ?";
                        ...
                        if ($stmt) {
                            // Bind parameters
                            $stmt->bind_param("iii", $condominium_id, $offset, $rowsPerPage);
        ...
        <div class="pagination justify-content-center mt-3">
            <ul class="pagination">
                <?php
                $total_pages_query = "SELECT COUNT(*) as total FROM payments WHERE is_deleted = 0 AND condominium_id = ?";
                $stmt_total_pages = $mysqli->prepare($total_pages_query);

                if ($stmt_total_pages) {
                    $stmt_total_pages->bind_param("i", $condominium_id);
                    $stmt_total_pages->execute();
                    $result = $stmt_total_pages->get_result();
