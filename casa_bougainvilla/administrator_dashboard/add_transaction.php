...
$residentCondoQuery = "SELECT users.account_number, users.username, condominiums.name 
                   ...
                   WHERE users.role = 'Resident' AND users.condominium_id = 1";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Insert into resident_transactions table
    $insert_query = "INSERT INTO resident_transactions (bill_number, account_number, billing_period_start, billing_period_end, due_date, total_amount_due, status, is_deleted, username) VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?)";
    $stmt = $mysqli->prepare($insert_query);
    ...
    // Log the activity
    logActivity($_SESSION['username'], "Added transaction for resident: $residentUsername with Bill Number: $billNumber", $residentUsername);
