...
if (isset($_POST['searchButton'])) {
    ...
    if (!empty($searchInput)) {
        ...
        $search_query = "SELECT * FROM activity_logs WHERE (timestamp LIKE ? OR user LIKE ? OR action LIKE ?) AND condominium_id = 1 ORDER BY timestamp DESC LIMIT $offset, $rowsPerPage";
...
$activity_logs_query = "SELECT * FROM activity_logs WHERE condominium_id = 1 ORDER BY timestamp DESC LIMIT $offset, $rowsPerPage";
$activity_logs_result = $mysqli->query($activity_logs_query);

// Calculate the total number of pages
$totalRows = $mysqli->query("SELECT COUNT(*) as count FROM activity_logs WHERE condominium_id = 1")->fetch_assoc()['count'];
...
if ($timestampFilter !== 'all') {
    $activity_logs_query = "SELECT * FROM activity_logs WHERE condominium_id = 1 AND timestamp >= NOW() - INTERVAL 1 $timestampFilter ORDER BY timestamp DESC LIMIT $offset, $rowsPerPage";
}
