...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($password !== $confirm_password) {
        ...
        echo "  window.location = 'add_administrator.php';";
        echo "}";
        echo "</script>";
        exit();
    }
    ...
    $dashboard_url = ($role === 'Administrator') ? 'casa_bougainvilla/administrator_dashboard/administrator_dashboard.php' : '';
    ...
    if ($stmt->errno === 0) {
        ...
        // Log the activity
        logActivity($_SESSION['username'], "Added Administrator: $name");
        ...
        $_SESSION['error'] = 'Error adding Administrator: ' . $stmt->error;
    }
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-4 offset-md-4 form">
                <form action="add_administrator.php" method="post" enctype="multipart/form-data">

                    <h2 class="text-center">Add Administrator</h2><br>

                    <div class="form-group">
                        <label for="username">Name:</label>
                        <input type="text" class="form-control" placeholder="Enter Name" name="username" autocomplete="off" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="password" class="form-control" placeholder="Re-enter Password" name="confirm_password" autocomplete="off" required>
                    </div>

                    <div class="form-group">
                        <label for="condominium">Select Condominium:</label>
                        <select class="form-control" name="condominium" autocomplete="off" required>
                            <?php
                            // To select condo names from table condominiums
                            $condo_query = "SELECT name FROM condominiums";
                            $condo_result = $mysqli->query($condo_query);

                            while ($row = $condo_result->fetch_assoc()) {
                                $condo_name = $row['name'];
                                echo "<option value=\"$condo_name\">$condo_name</option>";
                            }
                            ?>
                        </select>
                    </div><br>
